...
if($akMin <= 59){
    $statement = $pdo->prepare("SELECT AVG(Produktivitaet) AS Produktivitaet FROM ProduktivitaetVerlauf_Maschine WHERE DAY(Timestmp) = $akDay AND HOUR(Timestmp) = $akHour AND MINUTE(Timestmp) = $akMin AND MaschinenID = 3");
    $statement->execute();  
    $row = $statement->fetch();
    ...
    array_push($data, ["Maschine" => 3, "stunde" => $akHour, "minute" => $akMin, "Produktivitaet" => $zeitProduktivitaet]);
    $akMin = $akMin + 1;
